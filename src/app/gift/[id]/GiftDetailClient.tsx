...
import { ArrowLeft, ExternalLink, Edit2, Heart, Gift } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useTransition } from "react";
import Image from "next/image";
import { updateGiftStatus } from "@/actions/gift";
...
export default function GiftDetailClient({ gift, role }: { gift: GiftData; role: string }) {
    ...
    const handleMarkBought = () => {
        startTransition(async () => {
            await updateGiftStatus(gift.id, "bought");
            router.refresh();
        });
    };
                        ...
                        >
                            ...
                            {gift.notes && (
                                <div className="rounded-2xl border border-pink-100 bg-white/60 p-5 backdrop-blur-sm">
                                    ...
                                    <p className="text-slate-700">{gift.notes}</p>
                                </div>
                            )}
                            ...
                            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                                ...
                                {gift.status === "wishing" && (role === "prince" || role === "myfiance") && (
                                    <button
                                        ...
                                        disabled={isPending}
                                        className="flex flex-1 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-pink-400 to-pink-500 px-6 py-4 font-semibold text-white shadow-lg shadow-pink-500/30 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-pink-500/40 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0"
                                    >
                                        <Heart className="h-5 w-5" />
                                        {isPending ? "Đang lưu..." : "Đánh dấu đã chốt đơn"}
                                    </button>
                                )}
                            </div>
                        </m.div>
